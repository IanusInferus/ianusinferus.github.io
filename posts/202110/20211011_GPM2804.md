...
需要一台安装好Linux系统的PC，并且有读卡器或者MicroSD转USB适配器。
...
将游戏移动到本地硬盘的临时文件夹
    ...
    mkdir /tmp/roms
    mv /mnt/sdcard/home/pi/RetroPie/roms/* /tmp/roms
...
修复文件系统错误，如果错误太多可以加-y再运行

    e2fsck -f /dev/sdb2

缩减文件系统大小，由于resize2fs的局限性，指定大小太小时可能会失败，所以调到最小的可能大小(仍然会有很多剩余空间)。resize2fs结果会显示类似 `The filesystem on /dev/sdb2 is now 1055200 (4k) blocks long.`，即新的大小为 1055200 * 4k = 4.03GB
...
修改分区大小，第一个参数2表示/dev/sdb2的2，第二个参数表示分区结束位置+1，需要超过第二个分区的起始位置与文件系统大小之和
    ...
    parted /dev/sdb resizepart 2 5104467968B
...
使用剩余空间创建新分区，起始需要为1MB(1024*1024)的整数倍
    ...
    parted /dev/sdb mkpart primary ext4 5105516544B 100%
...
如果不为微雪官方镜像需要手工对照已有的项修改PARTUUID，保存重启，即可
