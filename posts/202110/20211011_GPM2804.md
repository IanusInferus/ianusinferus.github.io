...
<del>MicroUSB无法连接键盘等外设，似乎只能供电。主板上三个开关的功能似乎分别控制是否使用USB接口、SD卡、按键。需要额外的IO板才能刷写eMMC(如果CM4带eMMC)或更换EEPROM固件。</del>主板上有三个开关：S1 S2 S3全部设置为1，为正常模式；S1 S2 S3全部设置为0，为eMMC烧写模式，应该可以进行eMMC和EEPROM固件的烧写；S1设置为0，S2设置为1，S3设置为0，为USB外设模式，此时USB供电和自带手柄失效，可以连接键盘、U盘或网卡等。
...
MicroSD卡插槽和外壳之间有空隙，容易把MicroSD卡插到机器内部去，需要拆开外壳拿出。
